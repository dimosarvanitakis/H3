...
def MoveToCold(h3_cache, h3_cold, expires_time):
    """
    ...
    for h3_cache_bucket in h3_cache.list_buckets():
        ...
        for h3_cache_object in h3_cache.list_objects(h3_cache_bucket):
            ...
            cached_at = struct.unpack('d', h3_cold.read_object_metadata(h3_cache_bucket, h3_cache_object, "CachedAt"))

            # the object must be moved back to the cold storage
            if cached_at and (cached_at[0] + expires_time) >= now:
                done   = False
                offset = 0
                ...
                h3_cold.delete_object_metadata(h3_cache_bucket, h3_cache_object, "CachedAt")
...
def main(cmd=None):
    ...
    parser.add_argument('--expires_time', required=False, help=f'The maximum time that an object allowed to stay in the cache. (in seconds)')
    ...
    if cache_config_path and cold_config_path:
        ...
        if not expires_time:
            expires_time = DEFAULT_EXPIRES_AT_TIME

        # Move the objects from the cache storage to the cold storage   
        MoveToCold(h3_cache, h3_cold, expires_time)
    else:
